...
- GitHub/Dependabot still reports development dependency alerts from the old Create React App toolchain.
...
- Updated compatible React 18 / React Router 6 / TypeScript-era package versions while keeping the app on Create React App for now.
- Verified the cleanup with build, lint, test, and production audit checks.

## Known Risks

- `react-scripts@5.0.1` is stale and pulls vulnerable development-only transitive dependencies.
- The GitHub Actions workflow is old and does not pin Node or use a deterministic install.
- Test coverage is very thin and does not meaningfully cover the portfolio navigation or Zomboozled game flow.
- The README still contains stock Create React App content and should be rewritten once the build tool migration is complete.
...
1. Migrate from Create React App to Vite.
   ...
   - Preserve current routing and GitHub Pages deployment behavior.
   - Keep the UI behavior unchanged.
   - Re-run build, lint, tests, and dependency audit after migration.

2. Modernize GitHub Actions.
   - Use current action versions.
   - Pin a Node version.
   - Prefer deterministic install behavior.
   - Keep deploy behavior equivalent.

3. Add basic smoke coverage.
   - Cover app rendering.
   - Cover portfolio navigation.
   - Cover Zomboozled game-over/offline high-score behavior.

4. Rewrite the README.
   - Replace stock CRA content.
   - Explain the portfolio purpose, local development, deployment, and Zomboozled backend status.

5. Consider git history anonymization.
