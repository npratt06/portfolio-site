...
export default class HighScores extends Component<HighScoresProps, HighScoresState> {
    ...
    getScoreListItems(items: HighScoreItem[]) {
        const scoreListItems = items.map((item, index) => {
            ...
            return (<li key={index} style={{...listItemstyle}}>{displayStr}</li>);
        });
        ...
        if (this.state.scoreSubmitted) {
            inputComponents = (
                <div style={rowElement}>
                    {`Congrats ${this.state.newScore.name}, your high score has been submitted!`}
                </div>
            );
        } else {
            inputComponents = (
                <div>
                    <div style={rowElement}>
                        Your score: {this.props.newScore}
                    </div>
                    <div style={rowElement}>
                        You got a new high score! Enter your name: <input id="name-input" maxLength={6} onChange={(ev) => { this.onInputChange(ev) }} style={{ color: '#000000' }} type='text' defaultValue={this.state.newScore.name}></input>
                    </div>
                    <div style={rowElement}>
                        <button style={{ color: '#000000' }} onClick={() => { this.addNewScoreToDB(this.state.newScore) }}>
                            Submit high score
                        </button>
                    </div>
                </div>
