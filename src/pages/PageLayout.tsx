...
export default class PageLayout extends Component<PageLayoutProps, PageLayoutState> {
  constructor(props: PageLayoutProps) {
    ...
    pages.push(<Route key={ZomboozledPage.key} path={ZomboozledPage.linkPath} element={ZomboozledPage.component} />);
  ...
  componentDidUpdate(prevProps: Readonly<PageLayoutProps>, prevState: Readonly<PageLayoutState>): void {
    ...
    if (prevState.deviceType !== this.state.deviceType) {
      ...
      MyPages.pages.forEach((page) => {
        ...
        updatedPages.push(<Route key={page.key} path={page.linkPath} element={routeElement} />);
      });
      const routeElement = <Zomboozled deviceType={this.state.deviceType} />;
      updatedPages.push(<Route key={ZomboozledPage.key} path={ZomboozledPage.linkPath} element={routeElement} />);
